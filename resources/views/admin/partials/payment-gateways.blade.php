...
<div class="assist-admin-card glass-panel" id="payment-gateways">
    ...
    <p class="assist-text-muted mb-4">Choose the default gateway for new subscriptions. CheckoutPay supports bank transfer; Paystack uses card/USSD (NGN).</p>

    <form method="POST" action="{{ $gatewaySaveRoute ?? route('admin.assist.system.payment-gateway') }}" class="assist-admin-form-grid" style="max-width: 640px; margin-bottom: 28px;">
        @csrf
        <div class="assist-field">
            ...
            <select id="payment_gateway" name="payment_gateway" class="assist-select">
                <option value="checkoutpay" @selected($defaultGateway === 'checkoutpay')>CheckoutPay</option>
                <option value="paystack" @selected($defaultGateway === 'paystack')>Paystack</option>
            </select>
        </div>
        <div>
            <button type="submit" class="assist-btn assist-btn-primary">Save default gateway</button>
        </div>
    </form>
