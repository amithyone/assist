...
<div class="assist-auth-wrap">
    <div class="glass-panel assist-auth-card">
        ...
        <h1>{{ $i['heading'] ?? 'Reset password' }}</h1>
        <p class="assist-auth-sub">{{ $i['subheading'] ?? 'We will email you a reset link' }}</p>
        <x-assist.alert />
        ...
        <p class="assist-auth-links"><a href="{{ route('login') }}">Back to log in</a></p>
    </div>
</div>
@endsection
